...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // This will create a deployment called '[CONTRACTNAME]'. By default it will look for an artifact with the same name.
  // The 'contract' option allows you to use a different artifact.
  await deploy("MoonContract", {
    // Deployer will be performing the deployment transaction.
    from: deployer,
    // args[] is the address used as the first argument to the contract's constructor.
    args: [],
    log: true,
  });
};
